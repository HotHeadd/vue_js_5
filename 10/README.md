...
Как можно заметить, блокируется из за отсутствия CORS заголовка Access-Control-Allow-Origin. CORS -- это система безопасности в браузере. Тут браузер запритил запросы в удаленный ресурс потому что нет заголовка, разрешающего origin-запросы (запросы к другому домену.)
